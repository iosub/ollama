...
struct llm_build_qwen3vl : public llm_graph_context {
    llm_build_qwen3vl(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        // Extract deepstack embeddings if present (qwen3vl vision support)
        const int64_t n_embd_main = n_embd / 4;
        ggml_tensor * main_embed = nullptr;
        ggml_tensor * ds0 = nullptr;
        ggml_tensor * ds1 = nullptr;
        ggml_tensor * ds2 = nullptr;

        if (ubatch.embd) {
            // Image input: split inpL into main_embed and deepstack
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // Add deepstack for image inputs (fix illegal memory access for text-only)
            if (ubatch.embd) {
                switch (il) {
...
struct llm_build_qwen3vlmoe : public llm_graph_context {
    llm_build_qwen3vlmoe(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        // Extract deepstack embeddings if present (qwen3vlmoe vision support)
        const int64_t n_embd_main = n_embd / 4;
        ggml_tensor * main_embed = nullptr;
        ggml_tensor * ds0 = nullptr;
        ggml_tensor * ds1 = nullptr;

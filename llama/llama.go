...
func (b *Batch) Add(token int, embed []float32, pos int, logits bool, seqIds ...int) {
	...
	if b.nPosPerEmbd == 1 {
		...
		b.mropePos[n+allocSz] = C.llama_pos(pos)   // y
		b.mropePos[n+allocSz*2] = C.llama_pos(pos) // x
...
func (b *Batch) AddImageMRoPE(embeddings []float32, pos0 int, nx int, ny int, logitsLast bool, seqIds ...int) {
	...
	for y := 0; y < ny; y++ {
		for x := 0; x < nx; x++ {
			...
			b.mropePos[tokenIdx+nTokensFinal] = C.llama_pos(pos0 + y)    // y
			b.mropePos[tokenIdx+nTokensFinal*2] = C.llama_pos(pos0 + x)  // x
	...
	for i := range int(nChunks) {
		...
		if C.mtmd_input_chunk_get_type(chunk) == C.MTMD_INPUT_CHUNK_TYPE_TEXT {
			...
			if usesMRoPE && nx > 0 && ny > 0 {
				outChunks = append(outChunks, MtmdChunk{Embed: allEmbeds, Nx: nx, Ny: ny})
			} else {
				// For non-M-RoPE models, create separate chunks for each embedding
				for j := range numTokens {
					outChunks = append(outChunks, MtmdChunk{Embed: allEmbeds[j*numEmbed : (j+1)*numEmbed]})
				}
			}
		}
	}

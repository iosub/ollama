...
function Download-File {
    ...
    try {
        ...
        $response = Invoke-WebRequest -Uri $Url -OutFile $DestinationPath -PassThru
        
        if (Test-Path $DestinationPath) {
            $fileSize = (Get-Item $DestinationPath).Length / 1MB
            Write-Host "✅ Downloaded $FileName " -ForegroundColor Green -NoNewline
...
SYSTEM `"You are Qwen3-VL, a vision-language AI assistant. You can analyze images and answer questions about them. When given an image, describe what you see and answer any questions accurately.`"
"@

        Set-Content -Path $OutputPath -Value $modelfileContent -Encoding UTF8
...
function Register-Model {
    ...
    try {
        ...
        if (-not $ollamaCheck) {
            Write-Host "⚠️  Ollama not found in PATH. Please ensure Ollama is installed and accessible." -ForegroundColor Yellow
            return $false
        }
...
if (Confirm-Action "🔧 Do you want to register this model with Ollama now?") {
    if (Register-Model -ModelName $model.Name -ModelfilePath "Modelfile") {
        ...
        Write-Host "⚠️  Model downloaded but not registered. You can register it manually later." -ForegroundColor Yellow
    }
} else {
    Write-Host "📦 Model downloaded but not registered with Ollama." -ForegroundColor Yellow
    Write-Host "ℹ️  To register later, run: ollama create $($model.Name.ToLower()) -f Modelfile" -ForegroundColor Cyan
}

...
If you are using split GGUF models (e.g. Qwen3-VL), ensure that both the base model and the projector blobs are located in the `.ollama/models/blobs/` directory. The manifest must correctly reference the projector blob in the `ProjectorPaths` entry. If the projector blob is missing or corrupted, the model may fail to load or produce incorrect outputs.

...
func (m *VisionPositionEmbedding) Forward(ctx ml.Context, hiddenStates ml.Tensor, grid *Grid, opts VisionOptions) ml.Tensor {
	...
	indices := ctx.Input().FromIntSlice(slices.Concat(indexSlice...), grid.Height*grid.Width*4)
	weights := ctx.Input().FromFloatSlice(slices.Concat(weightSlice...), 1, grid.Height*grid.Width*4)

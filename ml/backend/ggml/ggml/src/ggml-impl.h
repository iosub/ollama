...
inline bool ggml_can_fuse_subgraph(const struct ggml_cgraph *          cgraph,
    ...
    if (num_ops <= 0 || (size_t)num_ops > max_ops) {
        return false;
    }

    if (start_idx < 0 || start_idx + num_ops > cgraph->n_nodes) {
        return false;
    }

    std::array<int, max_ops> node_idxs;
    for (int i = 0; i < num_ops; ++i) {
        node_idxs[i] = start_idx + i;
    }

    if (outputs.size() == 0) {
        return ggml_can_fuse_ext(cgraph, node_idxs.data(), ops.begin(), num_ops);
    }

    if (outputs.size() > max_ops) {
        return false;
    }

    std::array<int, max_ops> output_idxs;
    int num_outputs = 0;
    for (int output_idx : outputs) {
        output_idxs[num_outputs++] = output_idx;
    }
